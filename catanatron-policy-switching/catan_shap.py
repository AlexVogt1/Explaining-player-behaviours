...
def get_group(features, group):
    ...
    if group == "group_1":
        ...
        group_2 ={
            ...
            "turn_features": rolled_feature + rolled_robber_features,
            ...
            "p0_dev_cards_played": p0_devs_played+p0_played_dev_in_turn,
            "p1_dev_cards_played": p1_devs_played,
        }
        return group_2
    elif group == "group_2_random":
        shuffled_features = sklearn.utils.shuffle(features,random_state=42)
        group_2_random_group = {
            'group_1': shuffled_features[:68],
            'group_2': shuffled_features[68:136],
            'group_3': shuffled_features[136:204],
            'group_4': shuffled_features[204:272],
            'group_5': shuffled_features[272:340],
            'group_6': shuffled_features[340:408],
            'group_7': shuffled_features[408:476],
            'group_8': shuffled_features[476:544],
            'group_9': shuffled_features[544:],
        }
        return group_2_random_group
    elif group == "group_2_bad":
        shuffled_features = sklearn.utils.shuffle(features,random_state=42)
        group_2_bad_group = {
            'group_1': p0_hand + p1_hand + port_features,
            'group_2': node_features + bank_features + p0_devs_played,
            'group_3': edge_features + rolled_feature + p1_has_army,
            'group_4': tile_features + p0_has_army + p1_longest_road_length,
            'group_5': p1_has_road + rolled_robber_features,
            'group_6': p0_has_road + p1_pieces,
            'group_7': p0_vps + p1_public_vps,
            'group_8': p0_pieces + p1_devs_played,
            'group_9': p0_longest_road_length + p0_played_dev_in_turn,
        }
        return group_2_bad_group
    elif group == "group_3":
        group_3 ={
            'board_features': bank_features + tile_features + port_features, 
            'graph_features': node_features + edge_features,
        ...
        small_strat_group ={
            ...
            "rolled_and_robber": rolled_feature+rolled_robber_features,
    ...
    elif group == "strat_random_group":
        shuffled_features = sklearn.utils.shuffle(features,random_state=42)
        strat_random_group = {
            'group_1': shuffled_features[:40],
            'group_2': shuffled_features[40:80],
            'group_3': shuffled_features[80:120],
            'group_4': shuffled_features[120:160],
            'group_5': shuffled_features[160:200],
            'group_6': shuffled_features[200:240],
            'group_7': shuffled_features[240:280],
            'group_8': shuffled_features[280:320],
            'group_9': shuffled_features[320:360],
            'group_10': shuffled_features[360:400],
            'group_11': shuffled_features[400:440],
            'group_12': shuffled_features[440:480],
            'group_13': shuffled_features[480:520],
            'group_14': shuffled_features[520:560],
            'group_15': shuffled_features[560:],
        }
        return strat_random_group
    elif group == "group_3_random":
        shuffled_features = sklearn.utils.shuffle(features,random_state=42)
        small_random_group = {
            'group_1': shuffled_features[:122],
            'group_2': shuffled_features[122:244],
            'group_3': shuffled_features[244:367],
            'group_4': shuffled_features[367:489],
            'group_5': shuffled_features[489:],
        }
        return small_random_group
    elif group == "group_3_bad":
        small_bad_group = {
            'group_1': p0_has_army + p1_hand + p0_longest_road_length + port_features,
            'group_2': edge_features + p1_public_vps + rolled_feature + rolled_robber_features,
            'group_3': p0_hand + tile_features + p0_vps + bank_features+p1_has_army,
            'group_4': p1_has_road + p0_pieces + p1_devs_played + node_features+p0_played_dev_in_turn,
            'group_5': p0_devs_played + p1_pieces + p1_longest_road_length + p0_has_road
        }
        # total_length = 0
        # for key, value in small_bad_group.items():
        #     length = len(value)
        #     print(f"Length of {key}: {length}")
        #     total_length += length
        # print(f"Total length: {total_length}")
        return small_bad_group
    else:
        return None
#   p0_has_army, p1_has_army, p0_played_dev_in_turn, edge_features, port_features, p1_has_road, p1_pieces, p0_hand, p1_public_vps, p0_longest_road_length, p0_devs_played, tile_features, p0_has_road, rolled_robber_features, p1_hand, bank_features, p1_devs_played, p0_pieces, p1_longest_road_length, rolled_feature, node_features, p0_vps
# bank_features + tile_features + port_features, node_features + edge_features,rolled_feature + rolled_robber_features+p0_played_dev_in_turn,p0_vps + p0_has_army + p0_has_road+ p0_hand + p0_pieces + p0_longest_road_length + p0_devs_played,p1_public_vps + p1_has_army + p1_has_road + p1_hand + p1_pieces +p1_longest_road_length + p1_devs_played
...
def prep_data(data:pd.DataFrame,features):
    ...
    dfy_train = df['action']
    print("dfx_train\n",dfx_train)
    # test_indexes =data[data.ep == 8].index
    test_indexes =data.index
...
def get_games(games_dir = './good_games/'):
    ...
    return combined_df
def get_single_game(games_dir='./good_games/', episode_number=8):
    """
    Retrieve the DataFrame for a specific episode from the games directory.
    
    Parameters:
        games_dir (str): Directory containing game data CSV files.
        episode_number (int): The episode number to retrieve data for.
    
    Returns:
        pd.DataFrame: DataFrame containing data for the specified episode, or None if not found.
    """
    # Traverse directory, including all subdirectories
    for root, _, files in os.walk(games_dir):
        for file in files:
            # Check if file name matches the naming scheme for the specified episode
            if file.startswith(f"episode_{episode_number}") and file.endswith("_data.csv"):
                try:
                    # Read the CSV file into a DataFrame
                    file_path = os.path.join(root, file)
                    df = pd.read_csv(file_path)
    ...
    print(f"No data found for episode_{episode_number}.")
    return None
...
def groupshap_single(env_config= None, approach:str ="independence", grouping =None, agent_path = "./logs/Catan_Switch_Exp/exp_019/latest_model_1000000_steps.zip", nsamples=None,nbatches=None):
    print("using grad", torch.is_grad_enabled())
    data, info_list, catan_features = run_switch_agent(env_config=env_config, agent_path=agent_path)
    features = ["ep","step",'obs', 'action']
    # df = pd.DataFrame(data=data, columns=features)
    ...
    group = get_group(catan_features, grouping)
    # print(group)
    df = get_single_game("./good_games/", episode_number= 8)
    # df = get_games("./good_games/")
    print("good games",df)
    print(ast.literal_eval(df['obs'][0]))
    #TODO: finish setting up the use of good games
...
if __name__ == '__main__':
    ...
    #                                                              nsamples = n_samples,
    #                                                              nbatches = n_batches)
    df_shapley, pred_explain, internal, timing, MSEv = groupshap_single(env_config=config, 
                                                                 approach= 'empirical', 
                                                                 grouping = group, 
                                                                 nsamples = n_samples,
                                                                 nbatches = n_batches)
